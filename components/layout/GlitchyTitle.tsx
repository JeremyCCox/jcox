    ...
    useEffect(()=>{
        ...
        const interval = setInterval(()=>{
            ...
            if (randVal > .95){
                ...
                let selected:ScrambleTitle = possibleTitle2.find(({title})=>title===selections[Math.floor(Math.random()*selections.length)])||possibleTitle2[0]
                setTitle2(selected)
            }else{
       ...
       <>
           ...
           <div className={'flex relative'}>
               ...
               {possibleTitle2.map((title,index)=>{
                   return(
                       <h2 style={title.style} className={title===title2?'absolute translate-x-[200px] transition-all ease-in duration-500':'absolute translate-x-[200px] translate-y-[1lh] opacity-0 transition-all ease-in duration-500'} key={"title1"+index}>
                           {`${title.title}`}
                       </h2>
                   )
               })}
